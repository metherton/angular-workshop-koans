describe('Typescript - functions', function() {
  ...
  it('should understand lambdas', function () {
    let person1 = {
      ...
      setName: function (name: string) {
        ...
        let setFirstAndLastName = function (firstName: string, lastName: string) {
  ...
  it('functional extensions arrays', function() {
    let d = 2, result: number[];
    result = Array.apply(null, {length: Math.pow(6, d)})
      .map(Number.call, Number)
      .map(function (n: number) {
        return parseInt(n.toString(6), 10);
      })
      .map(function (n: number) {
        let sum = d;
        while (n) {
